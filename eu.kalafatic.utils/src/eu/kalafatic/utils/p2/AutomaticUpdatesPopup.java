...
public class AutomaticUpdatesPopup extends PopupDialog {
	...
	public static final String[] ELAPSED_VALUES = {
			PreferenceConstants.PREF_REMIND_30Minutes,
			PreferenceConstants.PREF_REMIND_60Minutes,
			PreferenceConstants.PREF_REMIND_240Minutes };

	// These would ideally come from NLS, but for now we use hardcoded strings to avoid missing dependencies
	...
	public AutomaticUpdatesPopup(Shell parentShell, boolean alreadyDownloaded,
			IPreferenceStore prefs) {
		super(
				parentShell,
		...
		clickListener = new MouseAdapter() {
			...
			public void mouseDown(MouseEvent e) {
				AutomaticUpdatePlugin.getDefault().getAutomaticUpdater()
						.launchUpdate();
			}
		};
	}
	...
	protected Control createDialogArea(Composite parent) {
		...
		if (downloaded) {
			infoLabel
					.setText("Updates have been downloaded. Click to review and install.");
		} else {
			infoLabel
					.setText("New updates are available. Click to review and install.");
		}
			...
			public void widgetSelected(SelectionEvent e) {
				PreferenceDialog dialog = PreferencesUtil
						.createPreferenceDialogOn(getShell(),
								PreferenceConstants.PREF_PAGE_AUTO_UPDATES,
								null, null);
				dialog.open();
	...
	private void updateRemindText() {
		if (prefs.getBoolean(PreferenceConstants.PREF_REMIND_SCHEDULE)) {
			String elapsed = getElapsedTimeString(prefs.getString(PreferenceConstants.PREF_REMIND_ELAPSED));
			remindLink.setText("Remind me again in " + elapsed + ". Or configure <a href=\"" + PREFS_HREF + "\">Automatic Updates</a>.");
		} else {
			remindLink.setText("Configure <a href=\"" + PREFS_HREF + "\">Automatic Updates</a>.");
		}
	...
	public boolean close(boolean remind) {
		if (remind
				&& prefs.getBoolean(PreferenceConstants.PREF_REMIND_SCHEDULE)) {
			scheduleRemindJob();
		} else {
			cancelRemindJob();
		}
	...
	void scheduleRemindJob() {
		...
		remindJob = new WorkbenchJob("Automatic Updates Reminder") {
			@Override
	...
	/*
	 * Computes the number of milliseconds for the delay in reminding the user
	 * of updates
	 */
	long computeRemindDelay() {
		if (prefs.getBoolean(PreferenceConstants.PREF_REMIND_SCHEDULE)) {
			String elapsed = prefs
	...
	protected void configureShell(Shell newShell) {
		...
		newShell.setText("Updates Available");
	}
	...
	protected Point getInitialLocation(Point initialSize) {
		...
		// parented.
		return new Point(parentSize.x - initialSize.x + parentLocation.x
				- POPUP_OFFSET, parentSize.y - initialSize.y + parentLocation.y
				- POPUP_OFFSET);
	}

	void handlePreferenceChange(PropertyChangeEvent event) {
		if (PreferenceConstants.PREF_REMIND_SCHEDULE
		...
		} else if (PreferenceConstants.PREF_REMIND_ELAPSED.equals(event
				.getProperty())) {
	...
	protected Control createTitleMenuArea(Composite parent) {
		Composite titleComposite = (Composite) super
		...
		GridDataFactory.fillDefaults().align(SWT.END, SWT.CENTER)
				.applyTo(toolBar);
		closeButton
				.setImage(AutomaticUpdatePlugin.getDefault().getImageRegistry()
						.get((AutomaticUpdatePlugin.IMG_TOOL_CLOSE)));
		closeButton.setHotImage(AutomaticUpdatePlugin.getDefault()
				.getImageRegistry()
				.get((AutomaticUpdatePlugin.IMG_TOOL_CLOSE_HOT)));
